...
<head>
    ...
    <title>Rico Hot Dog | FoodRush</title>
    ...
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        // Colores Oficiales Rico Hot Dog
                        rico: '#E31B23',        // Rojo Principal
                        'rico-dark': '#B0151B', // Rojo Oscuro
                        'rico-yellow': '#FFC72C', // Amarillo
                        'rico-light': '#FFF5F5', // Fondo Rojo muy pálido
                        'rico-cream': '#FEFCF5', // Fondo Crema suave
                    ...
                    backgroundImage: {
                        'stripes': "repeating-linear-gradient(45deg, transparent, transparent 10px, rgba(255, 199, 44, 0.1) 10px, rgba(255, 199, 44, 0.1) 20px)"
                    }
                }
            }
        }
    ...
    <style>
        ...
        .side-filter-btn:hover { color: #E31B23; background-color: #FFF5F5; }
        .side-filter-btn.active { color: #E31B23; font-weight: 700; border-left-color: #E31B23; background-color: #FFF5F5; }
        ...
        .size-card:hover { border-color: #E31B23; transform: translateY(-2px); }
        .size-card.selected { border-color: #E31B23; background-color: #FFF5F5; box-shadow: 0 4px 12px rgba(227, 27, 35, 0.15); }
        .size-card i { color: #d1d5db; transition: color 0.2s; }
        .size-card.selected i { color: #E31B23; }
        
        .counter-btn { width: 30px; height: 30px; border-radius: 50%; border: 1px solid #e5e7eb; color: #E31B23; display: flex; align-items: center; justify-content: center; transition: all 0.2s; background: white; }
        .counter-btn:hover { border-color: #E31B23; background-color: #FFF5F5; }
        ...
        .rico-toggle:hover { background-color: #f9fafb; color: #374151; }
        .rico-toggle.active {
        ...
        .filter-tab.active { background-color: #E31B23; color: white; border-color: #E31B23; box-shadow: 0 4px 6px -1px rgba(227, 27, 35, 0.3); }
        .filter-tab:not(.active):hover { border-color: #E31B23; color: #E31B23; background-color: #FFF5F5; }
    </style>
</head>
<body class="flex flex-col min-h-screen text-slate-800 bg-[#FEFCF5]" onload="updateCartBadge()">
    ...
    <header class="bg-white border-b border-gray-100 sticky top-0 z-50">
        <nav class="bg-white/95 backdrop-blur-sm shadow-sm py-3 md:py-4 sticky top-0 z-50">
            <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto px-4">
                ...
                <div class="hidden md:flex items-center border border-gray-200 rounded-full px-4 py-2 w-72 bg-gray-50 focus-within:bg-white focus-within:border-rico focus-within:ring-1 focus-within:ring-rico transition-all">
                    <i class="fa-solid fa-magnifying-glass text-gray-400 mr-3"></i>
                    <input type="text" id="searchInput" placeholder="Buscar hot dog..." class="outline-none w-full text-sm bg-transparent" onkeyup="app.search(this.value)">
                </div>
    ...
    <div id="catalog-view" class="fade-in pb-10">
        <section class="flex flex-col md:flex-row h-auto md:h-[450px] w-full bg-white border-b border-gray-100">
            <div class="w-full md:w-2/5 flex items-center justify-center p-8 md:p-12 bg-white z-10 order-2 md:order-1 relative overflow-hidden">
                <div class="absolute inset-0 bg-stripes opacity-30"></div>
                <div class="flex flex-col items-center z-10">
                    <img src="https://upload.wikimedia.org/wikipedia/commons/e/e3/Rico_Hot_Dog_logo.png" 
                         onerror="this.src='https://seeklogo.com/images/R/rico-hot-dog-logo-6F26D869F1-seeklogo.com.png'"
                         alt="Rico Hot Dog Logo" class="h-32 md:h-56 object-contain drop-shadow-xl hover:scale-105 transition duration-500">
                 </div>
            </div>
            <div class="w-full md:w-3/5 relative bg-gray-900 h-64 md:h-auto order-1 md:order-2">
                <div class="slider-container" id="hero-slider">
                    <div class="slide active"><img src="https://images.unsplash.com/photo-1612392174298-ea153623655b?q=80&w=2940" alt="Hot Dog"></div>
                    <div class="slide"><img src="https://images.unsplash.com/photo-1599599810769-bcde5a1645f7?q=80&w=2942" alt="Fries and Hotdog"></div>
                    <div class="slide"><img src="https://images.unsplash.com/photo-1564923630403-2284b87c0041?q=80&w=2942" alt="Street Food"></div>
                </div>
        ...
        <div class="bg-white/95 backdrop-blur border-b border-gray-200 sticky top-[60px] md:top-[73px] z-30 shadow-sm">
            <div class="container mx-auto px-4 md:px-6 py-3 md:py-4 flex gap-3 overflow-x-auto no-scrollbar" id="top-filters">
                <button onclick="app.setCategory('Hot Dogs', this)" class="filter-tab active px-5 py-2 rounded-full font-semibold text-sm md:text-base border whitespace-nowrap">Hot Dogs</button>
                <button onclick="app.setCategory('Acompanantes', this)" class="filter-tab px-5 py-2 rounded-full font-semibold text-sm md:text-base border border-gray-200 text-gray-600 whitespace-nowrap">Papas y Fritos</button>
                <button onclick="app.setCategory('Bebidas', this)" class="filter-tab px-5 py-2 rounded-full font-semibold text-sm md:text-base border border-gray-200 text-gray-600 whitespace-nowrap">Batidas y Jugos</button>
            </div>
        </div>

        <div class="container mx-auto px-4 md:px-6 py-6 md:py-8 grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-8">
            <aside class="md:col-span-1 select-none bg-white p-4 md:p-5 rounded-xl border border-gray-200 h-fit md:sticky md:top-36 shadow-sm">
                <h3 class="font-bold text-lg md:text-xl text-slate-800 mb-4 font-heading flex items-center justify-between">
                    Filtros <i class="fa-solid fa-filter text-rico text-sm md:hidden"></i>
                </h3>
    ...
    <div id="product-detail-view" class="hidden fade-in container mx-auto px-4 md:px-6 py-6 md:py-10 max-w-6xl">
        <button onclick="app.goBack()" class="mb-6 flex items-center text-sm font-bold text-gray-500 hover:text-rico transition group">
            <div class="w-8 h-8 rounded-full bg-white shadow-sm flex items-center justify-center mr-2 group-hover:bg-rico group-hover:text-white transition">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16">
            <div class="flex flex-col gap-6 md:gap-8">
                <div class="relative bg-rico-light rounded-3xl flex items-center justify-center p-6 h-72 md:h-[500px] border border-red-100 overflow-hidden">
                ...
                <div id="size-section" class="w-full">
                    <h3 class="font-bold text-lg text-slate-800 mb-4 font-heading border-b border-gray-100 pb-2">Elige tu Opción</h3>
                    <div class="flex gap-3 md:gap-4 justify-center md:justify-start">
                        <div class="flex flex-col items-center gap-2 cursor-pointer group" onclick="app.selectSize('Solo')">
                            <div class="size-card selected" id="size-Solo"><i class="fa-solid fa-hotdog text-2xl"></i></div>
                            <div class="text-center leading-tight">
                                <span class="block text-sm font-bold text-slate-700">Solo</span>
                                <span class="block text-xs text-gray-500 font-medium" id="price-label-Solo">$0</span>
                            </div>
                        </div>
                        <div class="flex flex-col items-center gap-2 cursor-pointer group" onclick="app.selectSize('Combo')">
                            <div class="size-card" id="size-Combo"><i class="fa-solid fa-utensils text-2xl"></i></div>
                            <div class="text-center leading-tight">
                                <span class="block text-sm font-bold text-slate-700">Combo</span>
                                <span class="block text-[10px] text-gray-400">+ Papas y Refresco</span>
                                <span class="block text-xs font-bold text-rico mt-1" id="price-label-Combo">$0</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex flex-col h-full">
                <div class="mb-6">
                    <h2 class="text-xs font-bold text-rico mb-2 tracking-widest uppercase" id="detail-category-title">Hot Dog</h2>
                    ...
                    <div class="flex items-center mb-4 text-sm">
                        ...
                        <span class="text-gray-400 ml-2 text-xs font-medium">El Sabor de lo Nuestro</span>
                    </div>
                ...
                <div id="customization-options" class="flex-grow">
                    <h3 class="font-bold text-lg text-slate-800 mb-4 font-heading flex items-center gap-2">
                        <i class="fa-solid fa-bacon text-rico"></i> Agrega tus Ingredientes
                    </h3>
                    
                    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                        <div class="p-4 border-b border-gray-100">
                            <span class="text-sm font-bold text-slate-700 block mb-3">Salsas</span>
                            <div class="flex flex-wrap gap-2">
                                <button class="rico-toggle active" onclick="app.toggleSauce(this)"><i class="fa-solid fa-check"></i> Ketchup</button>
                                <button class="rico-toggle active" onclick="app.toggleSauce(this)"><i class="fa-solid fa-check"></i> Mayonesa</button>
                                <button class="rico-toggle active" onclick="app.toggleSauce(this)"><i class="fa-solid fa-check"></i> Mostaza</button>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between items-center p-3 md:p-4">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-orange-600"><i class="fa-solid fa-cheese"></i></div>
                                <div><span class="text-sm font-bold text-slate-700 block">Queso Rallado</span></div>
                            </div>
                            <div class="flex items-center gap-2"><button onclick="app.updateIngredient('cheese-qty', -1)" class="counter-btn"><i class="fa-solid fa-minus text-[10px]"></i></button><span id="cheese-qty" class="font-bold text-slate-700 w-4 text-center text-sm">1</span><button onclick="app.updateIngredient('cheese-qty', 1)" class="counter-btn"><i class="fa-solid fa-plus text-[10px]"></i></button></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-8 bg-white pt-4 border-t border-gray-100 sticky bottom-0 z-20 pb-4 shadow-[0_-5px_15px_rgba(0,0,0,0.02)] md:shadow-none">
                    ...
                    <button onclick="app.addToCartFromDetail()" class="w-full bg-rico hover:bg-rico-dark text-white font-bold py-3 md:py-4 rounded-xl shadow-lg shadow-red-900/20 text-lg transition transform hover:scale-[1.01] active:scale-95 flex items-center justify-center gap-3">
     ...
     <footer class="bg-[#E31B23] text-white mt-auto">
        <div class="container mx-auto px-6 py-12 flex flex-col md:flex-row justify-between items-start md:items-center">
            <div class="mb-8 md:mb-0">
                 <div class="flex items-center gap-2 mb-4 bg-white w-fit px-3 py-1 rounded shadow-lg transform -rotate-2">
                    ...
                    <span class="text-rico font-bold text-xl italic -ml-1">Rush</span>
                </div>
                ...
                <div class="flex gap-4">
                    <a href="#" class="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center hover:bg-white hover:text-rico transition"><i class="fa-brands fa-facebook-f"></i></a>
                    <a href="#" class="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center hover:bg-white hover:text-rico transition"><i class="fa-brands fa-instagram"></i></a>
                    <a href="#" class="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center hover:bg-white hover:text-rico transition"><i class="fa-brands fa-twitter"></i></a>
                </div>
            </div>
    ...
    <script>
        const app = {
            cart: JSON.parse(localStorage.getItem('foodrush_rico_cart')) || [],
            currentCategory: 'Hot Dogs',
            activeTypeFilters: [], activeSpecFilter: null, 
            currentProduct: null, currentQty: 1, currentSize: 'Solo',
            sizePrices: { Solo: 0, Combo: 0 },
            currentSlide: 0, slides: [], dots: [], slideInterval: null,

            products: [
                { id: 1, name: "Perro de Res", category: "Hot Dogs", type: "Res", price: 160, spec: "classic", img: "https://ricohotdog.com/wp-content/uploads/2020/09/Perro-de-Res.png", desc: "La tradicional salchicha de res con repollo, cebolla, salsas y un toque de queso." },
                { id: 2, name: "Perro de Cerdo", category: "Hot Dogs", type: "Cerdo", price: 145, spec: "classic", img: "https://ricohotdog.com/wp-content/uploads/2020/09/Perro-de-Cerdo.png", desc: "Deliciosa salchicha de cerdo con todos los hierros: repollo, maíz y salsas." },
                { id: 3, name: "Perro de Pollo", category: "Hot Dogs", type: "Pollo", price: 155, spec: "classic", img: "https://ricohotdog.com/wp-content/uploads/2020/09/Perro-de-Pollo.png", desc: "Salchicha de pollo ligera y sabrosa, acompañada de vegetales frescos." },
                { id: 4, name: "Doble Salchicha", category: "Hot Dogs", type: "Especial", price: 210, spec: "double", img: "https://ricohotdog.com/wp-content/uploads/2020/09/Doble-Salchicha.png", desc: "Para mucha hambre: dos salchichas (Res o Cerdo) en un mismo pan." },
                { id: 5, name: "Papas Fritas", category: "Acompanantes", type: "Fritos", price: 110, spec: "side", img: "https://ricohotdog.com/wp-content/uploads/2020/09/Papas-Fritas.png", desc: "Papas fritas crujientes, el acompañante perfecto." },
                { id: 6, name: "Aros de Cebolla", category: "Acompanantes", type: "Fritos", price: 130, spec: "side", img: "https://ricohotdog.com/wp-content/uploads/2020/09/Aros-de-Cebolla.png", desc: "Aros de cebolla dorados y crujientes." },
                { id: 7, name: "Batida de Zapote", category: "Bebidas", type: "Batida", price: 120, spec: "sweet", img: "https://ricohotdog.com/wp-content/uploads/2020/09/Batida-Zapote.png", desc: "Batida cremosa de zapote natural, hecha al momento." },
                { id: 8, name: "Batida de Lechosa", category: "Bebidas", type: "Batida", price: 110, spec: "sweet", img: "https://ricohotdog.com/wp-content/uploads/2020/09/Batida-Lechosa.png", desc: "Refrescante y dulce batida de lechosa." },
                { id: 9, name: "Jugo de Naranja", category: "Bebidas", type: "Jugo", price: 90, spec: "fruit", img: "https://ricohotdog.com/wp-content/uploads/2020/09/Jugo-Naranja.png", desc: "Jugo 100% natural de naranja." }
            ],
            
            init: function() { 
                this.updateCartBadge();
                this.initSlider();
                this.setCategory('Hot Dogs');
            },
            ...
            setCategory: function(cat, btn) {
                ...
                document.querySelectorAll('.filter-tab').forEach(b => { b.className = "filter-tab border border-gray-200 text-gray-600 px-5 py-2 rounded-full font-semibold hover:border-rico hover:text-rico transition whitespace-nowrap text-sm md:text-base"; });
                if(btn) btn.className = "filter-tab active bg-rico text-white px-5 py-2 rounded-full font-semibold shadow transition whitespace-nowrap text-sm md:text-base";
                ...
                items.forEach(item => {
                    ...
                    <div class="fade-in border border-gray-100 rounded-2xl p-4 md:p-6 flex flex-col items-center justify-between h-[340px] md:h-[360px] hover:shadow-xl hover:border-rico transition-all duration-300 cursor-pointer bg-white group relative overflow-hidden" onclick="app.openProductDetail(${item.id})">
                        <div class="h-40 md:h-48 w-full flex items-center justify-center mb-4 relative p-2">
                            ...
                            onerror="this.src='https://images.unsplash.com/photo-1619740455993-9e612d446305?q=80&w=2940'">
                        </div>
                        <div class="text-center w-full relative z-10">
                            <h3 class="font-bold text-base md:text-lg text-slate-800 mb-1 group-hover:text-rico transition line-clamp-1 font-heading">${item.name}</h3>
                            <p class="text-xs text-gray-400 mb-3 md:mb-4">${item.type}</p>
                            <div class="flex justify-between items-center w-full bg-gray-50 p-2 rounded-xl group-hover:bg-yellow-50 transition border border-transparent group-hover:border-yellow-100">
                                <span class="text-slate-800 font-bold px-2 text-lg">$${item.price}</span>
                                <button class="w-8 h-8 rounded-lg bg-white text-rico shadow-sm hover:bg-rico hover:text-white transition flex items-center justify-center border border-gray-100"><i class="fa-solid fa-plus"></i></button>
                            </div>
                        </div>
                    </div>`;
                    grid.innerHTML += html;
            ...
            openProductDetail: function(id) {
                ...
                this.currentQty = 1;
                this.currentSize = 'Solo';
                const base = product.price;
                this.sizePrices = { Solo: base, Combo: base + 150 };

                document.getElementById('detail-category-title').innerText = product.category;
                document.getElementById('detail-title').innerText = product.name;
                document.getElementById('detail-desc').innerText = product.desc;
                document.getElementById('detail-img').src = product.img;
                document.getElementById('detail-img').onerror = function() { this.src = 'https://images.unsplash.com/photo-1619740455993-9e612d446305?q=80&w=2940'; };
                document.getElementById('detail-qty').innerText = "1";
                document.getElementById('detail-price-display').innerText = `$${this.sizePrices.Solo}`;
                
                const sizeSection = document.getElementById('size-section');
                const custSection = document.getElementById('customization-options');
                
                if (product.category === 'Hot Dogs') {
                    sizeSection.classList.remove('hidden');
                    custSection.classList.remove('hidden');
                    document.getElementById('price-label-Solo').innerText = `$${this.sizePrices.Solo}`;
                    document.getElementById('price-label-Combo').innerText = `$${this.sizePrices.Combo}`;
                    this.selectSize('Solo');
                    
                    document.getElementById('corn-qty').innerText = "0";
                    document.getElementById('cheese-qty').innerText = "1";
                    document.querySelectorAll('.rico-toggle').forEach(t => t.classList.add('active'));
            ...
            updateTotal: function() {
                let unitPrice;
                if (this.currentProduct.category === 'Hot Dogs') unitPrice = this.sizePrices[this.currentSize];
            ...
            addToCartFromDetail: function() {
                let unitPrice = this.currentProduct.category === 'Hot Dogs' ? this.sizePrices[this.currentSize] : this.currentProduct.price;
                let details = this.currentProduct.category === 'Hot Dogs' ? this.currentSize : "Normal";
                ...
                let cart = JSON.parse(localStorage.getItem('foodrush_rico_cart')) || [];
                ...
                localStorage.setItem('foodrush_rico_cart', JSON.stringify(cart));
                this.updateCartBadge();
                Swal.fire({ icon: 'success', title: '¡Añadido!', showConfirmButton: false, timer: 1000, background: '#E31B23', color: '#fff', toast: true, position: 'top-end' });
                this.goBack();
            },
            updateCartBadge: function() { const cart = JSON.parse(localStorage.getItem('foodrush_rico_cart')) || []; const count = cart.reduce((sum, i) => sum + i.qty, 0); const badge = document.getElementById('cart-count'); badge.innerText = count; badge.classList.toggle('hidden', count === 0); }
        };
        app.init();
