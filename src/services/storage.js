...
const normalizeOrder = (order = {}, fallbackEmail = '') => {
  ...
  return {
    ...
    delivery_fee: Math.max(0, toNumber(order.delivery_fee ?? order.deliveryFee, 0)),
    delivery_tip: Math.max(0, toNumber(order.delivery_tip ?? order.driver_tip ?? order.deliveryTip ?? order.driverTip, 0)),
    driver_tip: Math.max(0, toNumber(order.driver_tip ?? order.delivery_tip ?? order.driverTip ?? order.deliveryTip, 0)),
    delivery_distance_km: Math.max(0, toNumber(order.delivery_distance_km ?? order.deliveryDistanceKm, 0)),
  ...
  normalizeOrder(
    {
      ...
      tenantId: orderPayload.tenantId || orderPayload.tenant_id,
      tenantName: safeString(orderPayload.tenantName || orderPayload.tenant_name || orderPayload.franquicias?.[0]),
      tenant_name: safeString(orderPayload.tenant_name || orderPayload.tenantName || orderPayload.franquicias?.[0]),
      tenant_ids: orderPayload.tenant_ids || [],
      franquicias: orderPayload.franquicias || [],
      total: orderPayload.total,
      direccion_entrega: orderPayload.direccion_entrega,
      pickup_label: orderPayload.pickup_label,
      ...
      delivery_tip: orderPayload.delivery_tip,
      driver_tip: orderPayload.driver_tip,
      delivery_distance_km: orderPayload.delivery_distance_km,
      creado_en: new Date().toISOString(),
      user_email: normalizeEmailKey(userEmail),
      user_name: userName,
      tipo_entrega: orderPayload.tipo_entrega || deliveryMode,
